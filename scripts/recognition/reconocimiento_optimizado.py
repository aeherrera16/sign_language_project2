...
"""
 RECONOCIMIENTO OPTIMIZADO CON MEJOR PRECISION
===============================================
Version mejorada con configuraciones optimizadas para mejor reconocimiento
"""
...
def main():
    ...
    try:
        if os.path.exists('model/best_model.h5'):
            ...
            print(" No se encontro el modelo. Ejecuta 'python train_model.py' primero")
            return
    except Exception as e:
        print(f" Error cargando modelo: {e}")
        return
    
    # 2. INICIALIZAR MEDIAPIPE CON CONFIGURACION OPTIMIZADA
    print(" Inicializando MediaPipe optimizado...")
    mp_hands = mp.solutions.hands
    mp_face = mp.solutions.face_mesh
    mp_drawing = mp.solutions.drawing_utils
    
    # Configuracion mas estricta para mejor calidad
    hands = mp_hands.Hands(
        static_image_mode=False,
        max_num_hands=2,
        min_detection_confidence=0.8,   # Mas alto para mejor calidad
        min_tracking_confidence=0.7     # Mas alto para mejor calidad
    )
    ...
    print(" MediaPipe inicializado con configuracion optimizada")
    ...
    prediction_buffer = deque(maxlen=15)  # Buffer mas grande
    confidence_buffer = deque(maxlen=15)
    last_stable_prediction = None
    stability_counter = 0
    
    # 5. FUNCION DE EXTRACCION MEJORADA
    def extraer_landmarks_robusto(results_hands, results_face):
        """Extraccion robusta y normalizada de landmarks"""
        landmarks = []
        
        # Manos (126 elementos) con normalizacion mejorada
        if results_hands and hasattr(results_hands, 'multi_hand_landmarks') and results_hands.multi_hand_landmarks:
            ...
            for hand in results_hands.multi_hand_landmarks[:2]:
                ...
                # Convertir a array numpy para normalizacion
                hand_landmarks = np.array([[lm.x, lm.y, lm.z] for lm in hand.landmark])
                
                # Normalizacion centrada en la muneca
        ...
        # Cara (1404 elementos) con normalizacion
    ...
    # 6. FUNCION DE PREDICCION MEJORADA
    def predecir_con_suavizado(landmarks):
        """Prediccion con suavizado temporal avanzado"""
        ...
        # Filtro de confianza mas estricto
        if confianza < 0.8:  # Umbral alto para mayor precision
        ...
        if len(prediction_buffer) >= 10:
            ...
            counter = Counter(list(prediction_buffer)[-10:])  # Ultimas 10 predicciones
            most_common = counter.most_common(1)[0]
            most_common_gesture, count = most_common
            
            # Si un gesto aparece al menos 7 de las ultimas 10 veces
            if count >= 7:
    ...
    print("   1.  Haz gestos claros y definidos")
    print("   2.  Manten la sena por 3-4 segundos")
    print("   3.  Buena iluminacion uniforme")
    print("   4.  Fondo contrastante y simple")
    print("   5.  Distancia de 60-80cm de la camara")
    ...
    while True:
        ...
        # Dibujar landmarks de manos con mejor visualizacion
        ...
        # Intentar prediccion
        prediccion_text = "Sin deteccion de manos"
        confianza_text = ""
        status_text = ""
            ...
            if np.any(landmarks[:126]):  # Si hay datos de manos
                ...
                if gesto:
                    ...
                    if status == "ESTABLE":
                        ...
                        color = (0, 255, 0)  # Verde - prediccion estable
        ...
        # Dibujar informacion en pantalla con mejor diseno
        overlay = frame.copy()
        ...
        if key == ord('q'):
            ...
            print("=" * 40)
            print("1. Posicion: Manos en el centro de la pantalla")
            print("2. Distancia: 60-80cm de la camara")
            print("3. Iluminacion: Uniforme, sin sombras")
            print("4. Fondo: Simple y contrastante")
            print("5. Gesto: Claro y mantenido por 3-4 segundos")
            print("6. Si falla: Verifica que el gesto este en el dataset")
            print("=" * 40)
    ...
    if detecciones > 0:
        ...
        print(f"   Tasa de precision: {precision_rate:.1f}%")
        
        if precision_rate < 30:
            print("\n🚨 BAJA PRECISION DETECTADA:")
            print(" RECOMENDACIONES:")
            print("   1. Ejecuta 'python refuerzo_rapido.py' para mejorar el dataset")
            print("   2. Asegurate de hacer gestos incluidos en el modelo")
            print("   3. Verifica las condiciones de iluminacion")
            print("   4. Reentrena el modelo con mas datos")
