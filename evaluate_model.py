            ...
            if file.endswith(".npy"):
                try:
                    ...
                    if sample.shape == (1530,):
                        ...
                        print(f"⚠️ Archivo ignorado por forma inválida: {file} ({sample.shape})")
...
def create_visualizations(cm, labels, y_test, y_pred_proba, report):
    ...
    ax1.set_title('Precisión por Clase', fontweight='bold')
...
def analyze_errors(y_test, y_pred, y_pred_proba, labels, X_test):
    ...
    print(f"\n❌ ANÁLISIS DE ERRORES ({len(incorrect_indices)} errores):")
    ...
    # Mostrar errores más frecuentes
...
def generate_report():
    ...
    if os.path.exists("evaluation/metrics.json"):
        ...
        print("⚠️ No se encontraron métricas guardadas")
        return
    
    # Leer análisis de errores
...
- Precisión: {metrics['accuracy']:.4f} ({metrics['accuracy']*100:.2f}%)
...
- error_analysis.json: Análisis de errores detallado
"""
...
def main():
    """Función principal"""
    print("🔍 EVALUACIÓN COMPRENSIVA DEL MODELO")
    print("=" * 50)
