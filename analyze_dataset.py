...
def analyze_dataset():
    ...
    for gesture in gestures:
        ...
        # Analizar tamaños de archivos y formas
        valid_samples = 0
        invalid_samples = 0
        sample_shapes = []
        ...
        print(f"  {gesture:20s}: {valid_samples:3d} válidas, {invalid_samples:2d} inválidas")
    ...
    print(f"  - Mínimo por gesto: {dataset_info['statistics']['min_samples_per_gesture']}")
    print(f"  - Máximo por gesto: {dataset_info['statistics']['max_samples_per_gesture']}")
    print(f"  - Promedio por gesto: {dataset_info['statistics']['mean_samples_per_gesture']:.1f}")
    print(f"  - Desviación estándar: {dataset_info['statistics']['std_samples_per_gesture']:.1f}")
    print(f"  - Tamaño total: {dataset_info['statistics']['total_size_mb']:.2f} MB")
...
def create_dataset_visualizations(gestures, samples_per_gesture, dataset_info):
    ...
    # 1. Distribución de muestras por gesto
    plt.figure(figsize=(15, 8))
    bars = plt.bar(range(len(gestures)), samples_per_gesture, alpha=0.7, color='skyblue')
    plt.title('Distribución de Muestras por Gesto', fontsize=16, fontweight='bold')
    plt.xlabel('Gestos')
...
def generate_recommendations(dataset_info):
    ...
    if std_samples > 20:
        recommendations.append("📊 Dataset desbalanceado: Considera equilibrar el número de muestras por gesto")
        print("  - Dataset desbalanceado detectado")
    
    if min_samples < 50:
        recommendations.append(f"📈 Aumentar datos: {min_samples} muestras es insuficiente. Recomendado: mínimo 50 por gesto")
        print(f"  - Aumentar muestras (mínimo actual: {min_samples})")
    ...
    # Verificar muestras inválidas
    total_invalid = sum(details['invalid_samples'] for details in dataset_info['gesture_details'].values())
    if total_invalid > 0:
        recommendations.append(f"🔧 Limpiar datos: {total_invalid} muestras inválidas encontradas")
        print(f"  - Limpiar {total_invalid} muestras inválidas")
    
    # Recomendaciones de entrenamiento
    total_samples = dataset_info['statistics']['total_samples']
    if total_samples < 500:
        recommendations.append("🚀 Dataset pequeño: Considera usar técnicas de aumento de datos")
    ...
    if dataset_info['statistics']['std_samples_per_gesture'] > 30:
        ...
        print("  - Usar división estratificada")
    ...
    if not recommendations:
        print("  ✅ El dataset está en buen estado")

def clean_invalid_samples():
    """Limpiar muestras inválidas del dataset"""
    
    print("\n🧹 LIMPIEZA DE MUESTRAS INVÁLIDAS")
    print("=" * 40)
    ...
    if total_removed == 0:
        print("✅ No se encontraron muestras inválidas")
    else:
        print(f"\n✅ Limpieza completada: {total_removed} archivos removidos")

def main():
    """Función principal"""
    print("🔬 ANÁLISIS Y OPTIMIZACIÓN DEL DATASET")
    print("=" * 60)
    ...
    if dataset_info:
        # Preguntar si limpiar datos inválidos
        print("\n" + "="*50)
        response = input("¿Deseas limpiar muestras inválidas? (s/n): ").lower().strip()
        
        if response in ['s', 'sí', 'si', 'y', 'yes']:
            clean_invalid_samples()
            print("\n🔄 Re-analizando después de la limpieza...")
            analyze_dataset()
