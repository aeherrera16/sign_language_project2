...
def construir_modelo_mejorado(num_clases):
    """Modelo mejorado con mejores técnicas de regularización"""
    
    model = tf.keras.Sequential([
        # Capa de entrada con normalización
        tf.keras.layers.Input(shape=(1530,)),
...
def data_augmentation(X, y, augment_factor=2):
    """Aumenta los datos aplicando pequeñas variaciones"""
...
def main():
    ...
    if min_samples < 50:
        print(f"\n🔄 Aplicando aumento de datos (mínimo: {min_samples} muestras)")
    ...
    print(f"  - Top-K Precisión: {top_k_acc:.4f}")
